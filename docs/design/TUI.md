...
- **鼠标点击**（`mouse.mjs`）：左键按下 `\x1b[<0;col;rowM` → picker 选项点击选中（跳过标题行，按 `_row` 映射 filteredItems）；对话区点击 → 折叠提示行直接展开 / 普通行弹出"Line actions"菜单（复制到系统剪贴板 / 载入输入框编辑）。坐标 1-based、col 在前；release/滚轮不消费。convLine 携带 `_src`（state.lines 索引），picker lines 携带 `_row`（item 序号）——由 buildConvLines/rebuildLines 生成，点击映射与渲染共用同一套布局数学（`convGlobalIndex` 与 renderConversation 同式）。
