...
| 指标 | ThinCoder | Kimi-Code | MiMo-Code | OpenCode |
|------|-----------|-----------|-----------|----------|
| **语言** | JS (`.mjs`) | TS | TS | TS |
| **构建步骤** | 无 | 有 (pnpm) | 有 (bun) | 有 (bun) |
| **核心代码行数** | ~6,000 | 88,716 | 103,126 | 33,210 |
| **总代码行数** | ~10,000 | ~200k+ | ~300k+ | ~100k+ |
| **npm 运行时依赖** | **0** | 29 | 110 | 64 |
| **核心源文件数** | ~82 `.mjs` + 19 `.md` 工具描述 | 627 `.ts` | 536 `.ts` | 316 `.ts` |
| **测试文件数** | 4（agent / memory / tools / tui） | ~100+ | ~200+ | ~100+ |
| **测试用例数** | **141** | 未统计 | 未统计 | 未统计 |
| **测试通过率** | **141/141** ✅ | 未测 | 未测 | 未测 |
| **版本** | v0.8.3 | — | — | — |

**结论**：ThinCoder 以 **1/10 ~ 1/20 的代码量** 覆盖了其他 agent 的核心功能，同时维持 **零 npm 依赖**。这不是"功能少"而是"同样功能用的代码少"——约 82 个源文件 + 19 个工具描述实现 28 个工具、主循环、TUI、三层记忆、子 agent、MCP、checkpoint。代码量优势来自：无类型体操、无抽象框架、无冗余设计。
...
| 维度 | ThinCoder | Kimi-Code | MiMo-Code | OpenCode |
|------|-----------|-----------|-----------|----------|
...
**每引入一个 npm 包就引进一份技术债**（CVE、版本冲突、维护成本）。ThinCoder 的零依赖策略是工程洁癖的极致，也是 agent 稳定性的护城河。
...
| 覆盖范围 | TUI 纯函数与布局（33）、键盘处理（12）、memory CRUD（15）、tools 文件/系统/git（23）、agent 循环（66） |
| 执行速度 | ~8s |

全部离线测试，不依赖真实 API——意味着 CI 零成本、任何环境都能跑。141 条覆盖了核心流程、边界条件、错误路径。
...
| 要最小认知负担 | **ThinCoder**（约 80 个源文件，一下午能读完所有代码） |
...
*本报告基于各项目截至 2026-07-27 的公开代码与文档。数据来自源码审计（行数统计、依赖分析、测试运行），非主观印象。*
