...
> 状态：讨论中。本文档随对话持续更新，定型后拆分为正式需求/设计文档。
...
| 运行平台 | Node.js >= 22（可定 24） | 可用 `node:sqlite` 等原生能力，免去存储依赖 |
| 界面 | TUI：裸 ANSI 转义 ✅ | 零依赖，终端控制自研（raw mode / 按键解析 / 宽字符，约几百行） |
| 模型兼容范围 | **只跟顶流、只跟最新**，不为了兼容老旧模型捆住手脚 | AI 产业迭代太快，兼容老旧模型是负资产。只支持国内顶流厂商（DeepSeek/Kimi/GLM/Qwen/MiniMax 等）的最新一代模型。不做 Ollama/本地模型适配、不做 Anthropic 原生协议、不做"通用 OpenAI 兼容端点"的泛化承诺。预设表随模型换代增删，不留历史包袱。 |
