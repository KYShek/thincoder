...
| 模型兼容范围 | **只跟顶流、只跟最新**，不为了兼容老旧模型捆住手脚 | AI 产业迭代太快，兼容老旧模型是负资产。只支持顶流厂商（DeepSeek/Kimi/GLM/Qwen/MiniMax/OpenAI/Claude/Gemini 等）的最新一代模型。不做 Ollama/本地模型适配。预设表随模型换代增删，不留历史包袱。**（08-13 变更注记：Anthropic/Google 原生协议与通用自定义端点后来已支持——`format: anthropic/google` 三个 transport 均已实现，此条"不做"收窄为仅指"不做老旧模型兼容"。）** |
