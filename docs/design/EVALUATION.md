...
| **TeamCode** | 同团队前期项目 | 面向团队的共享记忆编程 agent（已放弃） |

---

## 1. 代码规模与复杂度

| 指标 | ThinCoder | TeamCode | Kimi-Code | MiMo-Code | OpenCode |
|------|-----------|----------|-----------|-----------|----------|
| **语言** | JS (`.mjs`) | TS | TS | TS | TS |
| **构建步骤** | 无 | 有 | 有 (pnpm) | 有 (bun) | 有 (bun) |
| **核心代码行数** | 8,726 | 6,222 | 88,716 | 103,126 | 33,210 |
| **总代码行数** | 13,852 | ~10k | ~200k+ | ~300k+ | ~100k+ |
| **npm 运行时依赖** | **0** | 9 | 29 | 110 | 64 |
| **核心源文件数** | 16 `.mjs` | 41 `.ts` | 627 `.ts` | 536 `.ts` | 316 `.ts` |
| **测试文件数** | 2 | ~10+ | ~100+ | ~200+ | ~100+ |
| **测试用例数** | **113** | 未统计 | 未统计 | 未统计 | 未统计 |
| **测试通过率** | **113/113** ✅ | 未知（项目已弃） | 未测 | 未测 | 未测 |
| **版本** | v0.7.3 | — | — | — | — |

**结论**：ThinCoder 以 **1/10 ~ 1/20 的代码量** 覆盖了其他 agent 的核心功能，同时维持 **零 npm 依赖**。这不是"功能少"而是"同样功能用的代码少"——16 个源文件实现 19 个工具、主循环、TUI、三层记忆、子 agent、MCP。代码量优势来自：无类型体操、无抽象框架、无冗余设计。
...
| 特性 | ThinCoder | Kimi-Code | MiMo-Code | OpenCode | TeamCode |
|------|-----------|-----------|-----------|----------|----------|
| 工具调用循环 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 循环上限 | 100 轮 | ✅ | ✅ | ✅ | ✅ |
...
| 工具 | ThinCoder | Kimi-Code | MiMo-Code | OpenCode |
|------|-----------|-----------|-----------|----------|
...
**ThinCoder 在工具数量最多**，但做到这一点没有引入任何额外依赖。三个检索工具（repo_outline / code_search / doc_search）是独创：它们从"结构 → 意图 → 细节"三层逐级深入，比传统的"给模型一个 grep"精确得多。

### 2.3 记忆系统

| 维度 | ThinCoder | Kimi-Code | MiMo-Code | OpenCode | TeamCode |
|------|-----------|-----------|-----------|----------|----------|
| 记忆范围 | Personal / Project / Team | ❌（session only） | Personal + Project | ❌ | Team / Project / Personal |
| 存储 | SQLite FTS5 | ❌ | SQLite FTS5 | ❌ | PostgreSQL pgvector |
| 向量检索 | ✅（RRF 混合） | ❌ | ❌（仅 BM25） | ❌ | ✅（RAG） |
| 语义检索 | ✅（BGE-M3） | ❌ | ❌ | ❌ | ✅ |
| Git 同步 | ✅（Project+Team） | ❌ | ❌ | ❌ | ❌ |
| 条目格式 | Markdown + frontmatter | ❌ | JSON/SQLite | ❌ | 数据库 |
| 人审机制 | ✅（distill y/n） | ❌ | ✅（dream/distill） | ❌ | ✅ |
| 冲突处理 | 结构规避 + 诚实报错 | ❌ | ❌ | ❌ | 未实现 |
| 检索隔离 | 按项目路径 | ❌ | ❌ | ❌ | 按 team |
| 必需外部服务 | **无** | ❌ | ❌ | ❌ | PostgreSQL |

**这是 ThinCoder 最核心的差异化优势**。TeamCode 愿景相同但倒在实现上。ThinCoder 是唯一一个把"团队记忆"真实跑通的项目——`verify-team.mjs` 脚本验证了 A→git→B 全链路。关键设计决策：
...
| 特性 | ThinCoder | Kimi-Code | MiMo-Code | OpenCode |
|------|-----------|-----------|-----------|----------|
...
| 特性 | ThinCoder | Kimi-Code | MiMo-Code | OpenCode |
|------|-----------|-----------|-----------|----------|
| 实现方式 | 裸 ANSI（零依赖） | pi-tui（React-like） | 基于 OpenCode | 自研 |
| 斜杠命令 | 全部菜单化 | ✅ | ✅ | ✅ |
| todo 面板 | ✅ | ✅ | ✅ | ❌ |
| 权限预览 | ✅（紧挨输入框） | ✅ | ✅ | ✅ |
| 子 agent 面板 | ✅ | ✅ | ✅ | ❌ |
...
TeamCode:   9 npm deps + PostgreSQL
...
**每引入一个 npm 包就引进一份技术债**（CVE、版本冲突、维护成本）。teamcode 的失败就与依赖有关——`ai-sdk` 两个 provider 版本互掐导致不可用。ThinCoder 的零依赖策略是工程洁癖的极致，也是 agent 稳定性的护城河。
...
| 覆盖范围 | TUI 纯函数、memory CRUD、tools 文件操作、agent 循环、provider 协议、context 压缩、session 序列化、subagent、goal/verify/plan、checkpoint、bash 护栏、代码索引、文档索引、repo_outline、前缀缓存 |
| 执行速度 | ~11s |

全部离线测试，不依赖真实 API——意味着 CI 零成本、任何环境都能跑。113 条覆盖了核心流程、边界条件、错误路径。比较之下，teamcode 的教训是"写了没跑"——类型错误、依赖错位一跑就现形。
...
| **无 LSP 集成** | 代码理解靠 FTS5 + 向量 + JSDoc，不是 LSP 语义分析 |
...
| **CI/CD** | 无（本地测试+人工发布） |
| **Windows 路径** | 已适配 cmd.exe/进程组杀，但非首要支持平台 |
...
| 要最小认知负担 | **ThinCoder**（16 个源文件，一天能读完所有代码） |
...
| 里程碑 | TeamCode | ThinCoder |
|---------|----------|-----------|
| 团队记忆愿景 | 写了 PPT | **跑通了** |
| 零依赖 | 9 deps + PostgreSQL | **0 deps** |
| 可运行验证 | 写了没跑 | **每个里程碑真实验证** |
| 索引重建 | 依赖数据库 | **SQLite 是易失品** |

TeamCode 的愿景和 ThinCoder 完全相同——"一人学到，全队皆知"。TeamCode 用 PostgreSQL + pgvector 追求 RAG，结果倒在复杂性上。ThinCoder 用 SQLite + 暴力余弦 + RRF 同样实现了混合检索，而且零外部依赖。"薄"在这里不是简陋，是**用最低复杂度达到同等效果**。

### 6.3 一句话总结

| 项目 | 一句话 |
|------|--------|
| **ThinCoder** | 工程师的刀——克制、精准、零依赖。团队记忆是唯一真实跑通的差异化王牌。 |
...
| **TeamCode** | 有价值的教训——愿景对、执行错，倒在"依赖地狱 + 写了没跑"。 |

---

*本报告基于各项目截至 2026-07-25 的公开代码与文档。数据来自源码审计（行数统计、依赖分析、测试运行），非主观印象。*
