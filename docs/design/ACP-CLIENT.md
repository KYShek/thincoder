...
> 目标：`thincoder acp` 子命令——通过 [Agent Client Protocol](https://agentclientprotocol.com/)（0.23）
> 在 stdio 上暴露 thincoder agent，使 Zed / JetBrains AI Chat / Paseo 等 ACP 客户端可直接驱动。
> 实证参考：工作区 `kimi-code/packages/acp-adapter/`（`@moonshot-ai/acp-adapter`，方法覆盖 10/12 + 4/9）
> 与 `kimi-code/docs/en/reference/kimi-acp.md` 能力矩阵。
...
- 表面：**stable**（生产客户端必用）+ **unstable**（实验扩展，可全不做）
- 鉴权：`initialize` 返回 `authMethods`；`authenticate` 校验

## 3. 方法覆盖（thincoder 裁剪版）

### 3.1 Stable agent-side（IDE → agent）—— 11/12

| Method | 做 | 说明 |
|---|---|---|
| `initialize` | ✅ | 版本协商；返回 agentInfo、能力矩阵、authMethods（terminal） |
...
| `session/close`、`logout` | ❌ | close 由进程生命周期兜底；logout 不支持 |

### 3.2 Stable client-side reverse-RPC（agent → IDE）—— 5/9

| Method | 做 | 说明 |
|---|---|---|
| `session/update` | ✅ | 流式 `agent_message_chunk` / `tool_call*` / `plan` / `config_option_update` / `available_commands_update` |
...
### 3.3 Unstable surface —— 全不做（19/19）

含 buffer 同步、inline-edit 预测、provider 管理等——Zed/JetBrains 正常流程不依赖。
...
| runAgent 内部 | ACP 通知 |
|---|---|
| `callbacks.onToken(text)` | `session/update: agent_message_chunk` |
| `callbacks.onReasoning` | `agent_message_chunk`（thinking 标记） |
| 工具开始/结果（tool loop 内） | `tool_call_start` / `tool_call_update` / `tool_call_end` |
| plan mode 切换 | `plan` 事件（`session/update` 内 plan 块） |
| `callbacks.onUsage` | `usage_update`（token 统计） |
| `callbacks.onTurnEnd` | 回合结束通知 |
...
| 不做 | 理由 |
|---|---|
| unstable surface 18 方法 | 正常客户端流程不依赖 |
| terminal 反向 RPC | shell 本地执行（kimi 同取舍） |
| audio prompt | 无音频输入通道 |
| ACP 会话写存档 | 客户端管生命周期 |
| 依赖官方 SDK | 零依赖哲学；自写层 ~100 行可测 |
